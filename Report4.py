...
def input_write(folder_path):
    ...
    outputfile_path = os.path.join(output_folder_path, 'output.csv')
    ...
    if os.path.exists(outputfile_path):
        output_parent.remove_file(outputfile_path)
    output_parent.create_file('output.csv')
    
    with open(outputfile_path, 'w', encoding='utf-8-sig') as f:
        f.write('檔名,總封數,總張數\n')
    ...
    for file_name in files_and_dirs:
        ...
        if os.path.isfile(file_path):
            try:
                    ...
                    for line in f:
                        ...
                        if match:
                            ...
                            with open(outputfile_path, 'a', encoding='utf-8') as file:
                                file.write(f"{file_name},{pack_count},{print_count}\n")
                        else:
                            print("未找到匹配的數字")
                ...
                destination_path = os.path.join(move_folder_path, file_name)
                shutil.move(file_path, destination_path)
                print(f"{file_name} 已移動到 {move_folder_path}")
